...
import { defaultBreakPoints } from '../types/theme';
import { isBrowser } from '../utils';
function getQueryString(brkRule, brks) {
    return brks.map((brk) => {
        ...
        if (idx === defaultBreakPoints.length - 1)
            return `(min-width: ${min}px)`;
        return `(min-width: ${min}px) and (max-width: ${max}px)`;
    });
}
export function useMedias(brks, defaultMatched) {
    const theme = useTheme();
    const querys = computed(() => getQueryString(theme.breakpoints, unref(brks)));
    const matchedIdx = ref(-1);
    watch(querys, (_, _2, onInvalidate) => {
        if (!isBrowser)
            return;
        const uns = [];
        const mqlList = querys.value.map((query) => query ? window.matchMedia(query) : null);
